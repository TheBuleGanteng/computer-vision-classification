...
class ModelOptimizer:
    ...
    def _suggest_cnn_hyperparameters(self, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        # Suggest number of conv layers first
        num_layers_conv = trial.suggest_int('num_layers_conv', 1, 4)
        
        # FIX: Suggest kernel and pool sizes as integers instead of strings
        kernel_size_int = trial.suggest_categorical('kernel_size_int', [3, 5])
        pool_size_int = trial.suggest_categorical('pool_size_int', [2, 3])
        padding = trial.suggest_categorical('padding', ['same', 'valid'])
        
        # Convert to tuples (now these will be properly stored in the trial)
        kernel_size = (kernel_size_int, kernel_size_int)
        pool_size = (pool_size_int, pool_size_int)
        
        # Apply constraints AFTER suggestion using conditional logic
        # For deep networks (3+ layers), force 3x3 kernels to preserve spatial dimensions
        if num_layers_conv >= 3:
            kernel_size = (3, 3)
            logger.debug(f"running _suggest_cnn_hyperparameters ... Deep network ({num_layers_conv} layers): forcing 3x3 kernels")
        
        # Additional constraint: If using 'valid' padding with large kernels and many layers, force safer combinations
        if padding == 'valid' and kernel_size_int == 5 and num_layers_conv > 2:
            kernel_size = (3, 3)
            logger.debug(f"running _suggest_cnn_hyperparameters ... Overriding to 3x3 kernels for valid padding with {num_layers_conv} layers")
        ...
        max_epochs = min(30, self.config.max_epochs_per_trial)  # Respect max constraint
        ...
        logger.debug(f"running _suggest_cnn_hyperparameters ... Suggested CNN params:")
        ...
        logger.debug(f"running _suggest_cnn_hyperparameters ... - kernel_size: {kernel_size} (from int: {kernel_size_int})")
        logger.debug(f"running _suggest_cnn_hyperparameters ... - pool_size: {pool_size} (from int: {pool_size_int})")
        logger.debug(f"running _suggest_cnn_hyperparameters ... - padding: {padding}")
        logger.debug(f"running _suggest_cnn_hyperparameters ... - activation: {activation}")
        logger.debug(f"running _suggest_cnn_hyperparameters ... - global_pooling: {use_global_pooling}")
        logger.debug(f"running _suggest_cnn_hyperparameters ... - batch_norm: {batch_normalization}")
        logger.debug(f"running _suggest_cnn_hyperparameters ... - gradient_clipping: {enable_gradient_clipping}")
        logger.debug(f"running _suggest_cnn_hyperparameters ... - epochs: {epochs} (range: {min_epochs}-{max_epochs})")
        ...
        return {
            ...
            # Convolutional layers - FIX: Store tuples directly
            'num_layers_conv': num_layers_conv,
            'filters_per_conv_layer': filters_per_conv_layer,
            'kernel_size': kernel_size,  # Now this is already a tuple
            'pool_size': pool_size,      # Now this is already a tuple
    ...
    def _calculate_objective_value(
        ...
        if self.config.objective == OptimizationObjective.ACCURACY:
            ...
            for _ in range(3):  # Average over 3 runs
                ...
                model.predict(sample_x, verbose=1)
...
def optimize_model(
    ...
    **config_overrides
    ...
    Args:
        ...
        naming_string: Optional naming string for consistent directory/file naming
...
if __name__ == "__main__":
    ...
    naming_string = args.get('naming_string', None)
    ...
    if naming_string:
        logger.debug(f"running model_optimizer.py ... Naming string: {naming_string}")
    ...
    try:
        ...
        result = optimize_model(
            ...
            trials=trials,
            naming_string=naming_string,
            **{k: v for k, v in args.items() if k not in ['dataset', 'optimize_for', 'trials', 'naming_string']}
        )
